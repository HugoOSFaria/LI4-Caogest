  ...
  >
    ...
    <v-card>
      <v-card flat color = "deep-orange lighten-4" class = "pa-6">
        <span class="display-1" dark>Alterar Concelho</span>
      </v-card>
      <v-card-text>
        <v-container>
          <v-col>
...
const lhost = require("@/config/global").host;
  ...
  export default {
    ...
    data: () => ({
      ...
      timeout: 4000,
      text: "", 
      regraLocalidade: [v => !!v || "Introdução de localidade obrigatória."],
    }),
    methods:{
      confirma: async function(){
        if (this.$refs.form.validate()) {
          try{ 
            ...
            this.dialog = false; 
            this.text = "Localidade alterada com sucesso!";
            this.color = "success"; 
            this.snackbar = true; 
          }
          catch(e){
