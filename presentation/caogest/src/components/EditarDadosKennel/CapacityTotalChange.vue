  ...
  >
    ...
    <v-card>
      <v-card flat color = "deep-orange lighten-4" class = "pa-6">
        <span class="display-1" dark>Alterar Capacidade Total</span>
      </v-card>
      <v-card-text>
        <v-container>
          <v-col>
            <v-text-field 
              color = "grey" 
              placeholder="Nova Capacidade Total" 
              required
              class = "ma-12"
              type="number"
...
const lhost = require("@/config/global").host;
  export default {
    ...
    data: () => ({
      ...
      timeout: 4000,
      text: "", 
      regracapacidadeTotal: [v => !!v || "Introdução de um valor obrigatório."],
    }),methods:{
